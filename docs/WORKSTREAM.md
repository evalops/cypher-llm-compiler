...
Status: implemented through `evaluateAttempts`, `cypher-llm eval`, importer helpers, checked-in text2cypher/openCypher samples, baseline reports, CypherBench report comparison, scorecard JSON/markdown output, benchmark gate reports, dataset governance reports, and repair-loop feedback packets.
...
The years-scale program is tracked publicly in issues #10-#17 and in the machine-readable roadmap under `src/years-roadmap.ts`, `schemas/years-roadmap.schema.json`, and `examples/roadmap/cypher-llm-years-roadmap.json`. Lossless parsing now has its first byte-preserving report lane under `src/lossless-parser.ts`, `schemas/lossless-parse.schema.json`, and `examples/lossless/tool-hash.lossless.json`. CypherBench now has its first public scorecard and gate lanes under `src/scorecard.ts`, `src/benchmark-gate.ts`, `schemas/cypherbench-scorecard.schema.json`, `schemas/benchmark-gate.schema.json`, `examples/benchmarks/tool-hash.scorecard.json`, and `examples/benchmarks/tool-hash.benchmark-gate.json`, plus dataset governance under `src/dataset-governance.ts`, `schemas/dataset-governance.schema.json`, and `examples/benchmarks/tool-hash.dataset-governance.json`. Dialect certification now has separated profile, renderer, parser, semantic, and live-database lanes under `src/dialect-certification.ts`, `schemas/dialect-certification.schema.json`, and `examples/certification/dialect-certification.json`. Semantic proof and repair planning now have compact artifacts under `src/proof.ts`, `src/repair-plan.ts`, `schemas/cypher-proof.schema.json`, `schemas/repair-plan.schema.json`, `examples/proofs/tool-hash.proof.json`, and `examples/proofs/tool-hash.repair-plan.json`. Cost and safety policy has report, planner-estimate, schema-statistics, policy-rule, and named-profile lanes under `src/policy.ts`, `src/planner-estimate.ts`, `src/schema-statistics.ts`, `src/policy-rules.ts`, `src/policy-profile.ts`, `schemas/policy-report.schema.json`, `schemas/planner-estimate.schema.json`, `schemas/schema-statistics.schema.json`, `schemas/policy-rules.schema.json`, `schemas/policy-profile.schema.json`, `examples/policy/tool-hash.policy.json`, `examples/policy/tool-hash.planner-estimate.json`, `examples/policy/tool-hash.schema-statistics.json`, `examples/policy/tool-hash.policy-rules.json`, and `examples/policy/policy-profiles.json`. Ecosystem UX has its first editor/agent diagnostics lane under `src/lsp.ts`, `schemas/lsp-diagnostics.schema.json`, and `examples/lsp/tool-hash.lsp.json`, plus the first machine-readable diagnostic-code catalog under `src/diagnostic-catalog.ts`, `schemas/diagnostic-catalog.schema.json`, and `examples/diagnostics/diagnostic-catalog.json`. The compiler-service workstream now has a runtime boundary plus a versioned manifest and operational controls under `src/http-server.ts`, `src/service-manifest.ts`, `schemas/service-manifest.schema.json`, `examples/service/service-manifest.json`, and `test/http-server.test.ts`. Governance now has compatibility catalog, diff-gate, and contract-conformance artifacts under `src/compatibility.ts`, `src/compatibility-diff.ts`, `src/contract-conformance.ts`, `schemas/compatibility-catalog.schema.json`, `schemas/compatibility-diff.schema.json`, `schemas/contract-conformance.schema.json`, `examples/governance/compatibility-catalog.json`, `examples/governance/compatibility-diff.json`, and `examples/governance/contract-conformance.json`.
