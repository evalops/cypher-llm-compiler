...
describe("cli", () => {
  ...
  it("prints and writes the compatibility catalog", async () => {
    const writes = new Map<string, string>();
    let stdout = "";
    let stderr = "";
    const io: CliIO = {
    ...
    const jsonCode = await runCli(["compatibility", "--integrity", "--fail-on-error", "--catalog-out", "out/compatibility.json"], io);
    const markdownCode = await runCli(["compatibility", "--format", "markdown"], io);
  ...
  it("prints and writes the agent guide", async () => {
    const writes = new Map<string, string>();
    let stdout = "";
    let stderr = "";
    const io: CliIO = {
    ...
    const jsonCode = await runCli(["agent-guide", "--guide-out", "out/agent-guide.json"], io);
    const markdownCode = await runCli(["agent-guide", "--format", "markdown"], io);

    assert.equal(jsonCode, 0);
    assert.equal(markdownCode, 0);
    assert.equal(stderr, "");
    assert.ok(stdout.includes("cypher-llm-agent-guide/v1"));
    assert.ok(stdout.includes("# Cypher LLM Agent Guide"));
